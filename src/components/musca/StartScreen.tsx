...
import { ArrowLeft, Check, Copy, Crown, LockKeyhole, MapPinned, RadioTower, Users } from "lucide-react";
...
export function StartScreen() {
  ...
  const createRoom = () => { void multiplayerClient.createRoom("", roomPassword, savePlayerName(), selectedMap); };
  const joinRoom = () => { void multiplayerClient.joinRoom(roomCodeInput, roomPassword, savePlayerName()); };
  const chooseRole = (slotId: MultiplayerSlotId) => multiplayerClient.claim(slotId, savePlayerName());
  const canCreate = roomPassword.length >= 1 && mp.status !== "connecting";
  const canJoin = roomCodeInput.replace(/[^A-Z0-9]/gi, "").length >= 4 && roomPassword.length >= 1 && mp.status !== "connecting";
    ...
    <div className={"start-card beta-start-card " + (entryMode === "menu" ? "classic-opening" : "session-opening")}>
      ...
      {entryMode === "menu" && <>
        <div className="opening-choice-grid">
          ...
          <button type="button" className="opening-choice" onClick={enterMultiplayer}><Users className="opening-choice-svg" /><strong>Multiplayer experimental</strong><small>Crie uma sala e compartilhe somente o código e a senha com seus amigos.</small></button>
        </div>
        <p className="opening-footnote">O jogo usa sinalização pública apenas para encontrar os jogadores; a partida segue em P2P pelo navegador.</p>
      </>}
      ...
      {entryMode === "multiplayer" && <div className="multiplayer-site-flow">
        {!mp.room.id && <>
          <div className="room-action-tabs" role="tablist" aria-label="Opções multiplayer"><button type="button" className={roomAction === "create" ? "active" : ""} onClick={() => setRoomAction("create")}>Criar sala</button><button type="button" className={roomAction === "join" ? "active" : ""} onClick={() => setRoomAction("join")}>Entrar em sala</button></div>

          {roomAction === "create" && <section className="session-section room-config-section">
            <div className="session-heading"><span>01</span><div><strong>Criar sala P2P</strong><small>O código é gerado automaticamente. Você só compartilha código e senha.</small></div></div>
            <div className="room-fields"><label><span>Senha</span><input maxLength={64} type="password" value={roomPassword} onChange={(e) => setRoomPassword(e.target.value)} placeholder="Senha da sala" autoComplete="new-password" /></label></div>
            ...
            <div className="session-map-grid">{MAPS.map((map) => <button key={map.id} type="button" className={"session-map-card " + (selectedMap === map.id ? "active" : "")} onClick={() => setSelectedMap(map.id)}><img src={map.preview} alt="" /><span><strong>{map.name}</strong><small>{map.difficulty} · {map.subtitle}</small></span>{selectedMap === map.id && <Check className="size-4" />}</button>)}</div>
            <button type="button" className="btn btn-primary room-submit" disabled={!canCreate} onClick={createRoom}>{mp.status === "connecting" ? "Criando sala…" : "Criar sala"}</button>
          </section>}

          {roomAction === "join" && <section className="session-section room-config-section">
            <div className="session-heading"><span>01</span><div><strong>Entrar em sala</strong><small>Digite o código enviado pelo anfitrião e a senha.</small></div></div>
            <div className="room-fields"><label><span>Código da sala</span><input maxLength={12} value={roomCodeInput} onChange={(e) => setRoomCodeInput(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))} placeholder="Ex.: K7M4Q2" autoComplete="off" /></label><label><span>Senha da sala</span><input maxLength={64} type="password" value={roomPassword} onChange={(e) => setRoomPassword(e.target.value)} placeholder="Senha" autoComplete="current-password" /></label></div>
            <button type="button" className="btn btn-primary room-submit" disabled={!canJoin} onClick={joinRoom}>{mp.status === "connecting" ? "Conectando…" : "Entrar"}</button>
          </section>}
          {mp.error && <p className="multiplayer-error room-error">{mp.error}</p>}
          <p className="section-note web-multiplayer-note"><RadioTower className="size-4" /> Beta P2P: o serviço de sinalização encontra os navegadores e a sessão é conectada diretamente entre os jogadores.</p>
        </>}
        ...
        {mp.room.id && <section className="session-section multiplayer-lobby site-room-lobby">
          ...
          <div className="multiplayer-connection"><span className={"status-dot " + mp.status} /><strong>{mp.status === "connected" ? "P2P ativo" : mp.status === "connecting" ? "Conectando…" : "Conexão perdida"}</strong></div>
          {mp.error && <p className="multiplayer-error">{mp.error}</p>}

          <div className="selected-mission-detail">
            <span>CÓDIGO DA SALA</span><strong style={{ letterSpacing: ".18em" }}>{mp.roomCode}</strong>
            <small>{mp.isHost ? "Envie este código e a senha para seus amigos." : "Você entrou usando este código."}</small>
            {mp.isHost && <button type="button" className="btn btn-ghost" onClick={copyRoomCode}>{copied ? "Copiado" : <><Copy className="size-4" /> Copiar código</>}</button>}
          </div>
